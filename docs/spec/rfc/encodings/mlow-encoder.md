...
[mlow-frame](../encodings/mlow-frame.md) and [mlow-rangecoder](../encodings/mlow-rangecoder.md)). The active
...
**Stage order (per packet).** The encoder MUST run, in order:
    ...
    1. SILK VAD over the int16-scaled input PCM (before the encoder high-pass),
       ...
       packet-level coded-as-active-voice flag (see [mlow-vad](../encodings/mlow-vad.md)).
...
[mlow-lsf-lpc](../encodings/mlow-lsf-lpc.md)). Conditional ("cond") coding against the previous
...
speech buffer) and the signal-mode classifier (see [mlow-vad](../encodings/mlow-vad.md)). The
...
enable bits (see [mlow-frame](../encodings/mlow-frame.md)) so the decoder emits silence or comfort
...
[in the full RFC →](../index.md#mlow-encoder) · [RFC contents](../index.md#contents)
