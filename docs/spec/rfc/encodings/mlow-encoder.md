...
[mlow-frame](../encodings/mlow-frame.md), [mlow-rangecoder](../encodings/mlow-rangecoder.md)). Active config-0
TOC byte is `0x50`.
...
**Stage order (per packet), in order:**
    1. SILK VAD over int16-scaled input PCM (before the high-pass), yielding a
       ...
       coded-as-active-voice flag (see [mlow-vad](../encodings/mlow-vad.md)).
...
`(grid, stage2[16])` (see [mlow-lsf-lpc](../encodings/mlow-lsf-lpc.md)). Use conditional ("cond")
...
signal-mode classifier (see [mlow-vad](../encodings/mlow-vad.md)). Fold five strengths into one
`voicing_strength`:
...
[mlow-frame](../encodings/mlow-frame.md)). A silent internal frame (autocorrelation lag-0 energy <= 0)
...
[← in the full RFC](../../../index.md#mlow-encoder)
