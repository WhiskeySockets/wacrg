...
**WhatsApp 1:1 call protocol** — signaling, keying, media, and transport. It is
...
## What are the six techniques?

`websocket-capture`, `baileys-instrumentation`, `frida-hooking`, `mitm-tls`,
`static-smali-analysis`, `memory-dump`. They reveal different layers; the
[techniques page](spec/techniques.md) details each one's strengths and limits.
...
techniques (Frida, memory dump), so it is the project's biggest open frontier.
