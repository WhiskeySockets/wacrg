...
This is the cheapest on-ramp and the recommended starting point. Because call
...
| --- | --- |
| `<call>` / `<offer>` / `<accept>` / `<terminate>` … node trees | the SRTP/RTP media plane (different transport) |
