...
SplitRED container (see [mlow-red-fec](../encodings/mlow-red-fec.md)). The main frame is the LAST
...
[mlow-frame](../encodings/mlow-frame.md)) and compute:
...
range-coded bitstream (see [mlow-rangecoder](../encodings/mlow-rangecoder.md)) covering all three
internal frames.
  ...
  2. For each internal frame `f` in `0, 1, 2`, read from the SAME range decoder in
     this exact order:
       ...
       a. LSF/LPC indices (see [mlow-lsf-lpc](../encodings/mlow-lsf-lpc.md)). Voiced iff the decoded
          LSF stage-1 index `== 1`.
  ...
  3. Run the per-packet harmonic post-filter (see [mlow-postfilter](../encodings/mlow-postfilter.md))
...
Breakdown: [`mlow-encoder`](../encodings/mlow-encoder.md)

**Implemented by**
- **whatsapp-rust** — working — pure-Rust stateful MlowDecoder; e2e decode matches the smpl_opus useSmpl reference at lag 0 · [commits ↗](https://github.com/oxidezap/whatsapp-rust/commits)
- **meowcaller** — partial — codec modules partial; full decode orchestration in progress · [commits ↗](https://github.com/purpshell/meowcaller/commits)
...
[← in the full RFC](../../../index.md#mlow-decoder)
