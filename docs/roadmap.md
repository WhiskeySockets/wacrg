...
wacrg is a **speedrun**: a small group of maintainers, each with a different
technique, racing to a complete, provenance-tracked spec of the WhatsApp 1:1 call
protocol. This page lays out the phases and how we use the
[coverage metric](spec/coverage.md) to decide what to chase next.
...
The under-served frontier. Confirm codecs, the SRTP profile, RTP details, and the
[ICE/relay](ice-and-relays.md) selection logic. This needs the higher-effort
techniques and is where the biggest `speculative → probable` jumps will happen.
...
## How we use coverage to speedrun
...
**by technique**. We use it as a live scoreboard:
...
- **Aim the next captures.** Open a `good-first-capture` issue targeting the
  weakest area so a new contributor can make an immediate dent.
- **Reward corroboration, not volume.** Because confirmed counts double-weight a
  probable, the metric naturally rewards a *second independent technique*
